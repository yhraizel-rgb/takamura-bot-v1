...
describe("Takamura Bot Pro API", () => {
  ...
  it("provides a demo token only in the test environment", async () => { const { app } = await createApp(); const response = await request(app).get("/api/v1/auth/demo"); expect(response.status).toBe(200); expect(response.body.mode).toBe("test"); expect(response.body.accessToken).toEqual(expect.any(String)); });
});
