...
export interface User { id: string; username?: string; email: string; passwordHash: string; role: Role; createdAt: string; }
export interface Session { id: string; phoneNumber: string; desired: "enabled" | "disabled"; state: SessionState; ownerUserId: string; features: Record<string, boolean>; qrCode?: string; pairingCode?: string; lastError?: string; updatedAt: string; }
