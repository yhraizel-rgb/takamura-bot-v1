...
export async function createApp() {
  ...
  if (env.AUTH_MODE === "test" && !store.value.users.some((user) => user.id === "demo-owner")) {
    store.value.users.push({ id: "demo-owner", username: "demo", email: "demo@takamura.local", passwordHash: await hashPassword("test-only-demo"), role: "Owner", createdAt: new Date().toISOString() });
    await store.save();
  }
  if (env.FIRST_OWNER_USERNAME && env.FIRST_OWNER_PASSWORD && !store.value.users.some((user) => user.username === env.FIRST_OWNER_USERNAME)) {
    store.value.users.push({ id: crypto.randomUUID(), username: env.FIRST_OWNER_USERNAME, email: env.FIRST_OWNER_EMAIL ?? `${env.FIRST_OWNER_USERNAME}@takamura.local`, passwordHash: await hashPassword(env.FIRST_OWNER_PASSWORD), role: "Owner", createdAt: new Date().toISOString() });
    await store.save();
  }
