...
export async function createApp() {
  ...
  app.get("/api/v1/auth/demo", (_req, res) => { if (env.AUTH_MODE !== "test") return res.status(404).json({ error: "NOT_FOUND" }); res.json({ accessToken: signAccessToken("demo-owner", "Owner"), user: { id: "demo-owner", email: "demo@takamura.local", role: "Owner" }, mode: "test" }); });
  ...
  app.post("/api/v1/sessions", requireAuth, requirePermission("sessions:create"), csrf, async (req, res) => { try { const session = await sessions.create(String(req.body?.phoneNumber ?? ""), req.user!.id); await sessions.pair(session.id); res.status(201).json({ data: { ...session, phoneNumber: `***${session.phoneNumber.slice(-4)}` } }); } catch (error) { logger.warn({ error, requestId: req.requestId }, "session.create.rejected"); res.status(400).json({ error: "INVALID_SESSION_REQUEST" }); } });
  ...
  app.get("/api/v1/stats", requireAuth, requirePermission("dashboard:read"), (_req, res) => res.json({ data: { bots: store.value.sessions.length, connected: store.value.sessions.filter((x) => x.state === "connected").length, commands: commands.length, users: store.value.users.length, queueDepth: 0 } }));
  app.get("/api/v1/logs", requireAuth, requirePermission("logs:read"), (_req, res) => res.json({ data: store.value.logs.slice(-100).reverse() }));
